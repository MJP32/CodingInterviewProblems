...
public class PrintTree {
    ...
    // Helper function #1
    private void dFSTraversal(TreeNode current, int height, int left, int right, int column) {
        if (current == null)
            return;
        ...
        if (height == 0) {
            ...
            dFSTraversal(current.left, height-1, left, mid-1, column);   // [left , mid-1]
            dFSTraversal(current.right, height-1, mid+1, right, column); // [mid+1, right]
        }
    }
